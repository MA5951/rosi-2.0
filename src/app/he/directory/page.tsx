...
const ContactsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchContacts = async () => {
            try {
                const fetchedContacts = await getAllContacts('hebrew', 'approved');
                const validatedContacts = validateAndDeduplicateContacts(fetchedContacts);
                setContacts(validatedContacts);
            } catch (error) {
                console.error('שגיאה בשליפת אנשי הקשר:', error);
                setError('נכשלה הטעינה של אנשי הקשר.');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return (
            <div dir="rtl" className="flex min-h-screen items-center justify-center bg-blue-50 dark:bg-slate-900 text-gray-900 dark:text-white p-4">
                <div className="w-full max-w-4xl mx-auto">
                    <h2 className="text-3xl font-bold mb-6 text-center">אנשי קשר</h2>
                    <div className="overflow-x-auto">
                        <table className="table-auto border-collapse w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg">
                            <thead>
                                <tr>
                                    <th className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300">
                                        מספר
                                    </th>
                                    <th className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300">
                                        שם
                ...
                <div className="overflow-x-auto">
                    <table className="table-auto border-collapse w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg">
                        <thead>
                            <tr>
                                <th
                                    className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300 cursor-pointer"
                                ...
                                <th
                                    ...
                                    onClick={() => requestSort('author')}
                                >
                                    שם {getSortIndicator('author')}
                                </th>
                                <th
                                    className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300 cursor-pointer"
                                    onClick={() => requestSort('phone')}
                                >
                                    טלפון / אימייל {getSortIndicator('phone')}
                                </th>
                                <th
                                    className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300 cursor-pointer"
                                    onClick={() => requestSort('teamnumber')}
                                >
                                    מספר קבוצה {getSortIndicator('teamnumber')}
                                </th>
                                <th
                                    className="border-b border-gray-300 px-6 py-3 bg-gray-100 dark:bg-gray-700 text-right font-bold text-gray-700 dark:text-gray-300 cursor-pointer"
                                    onClick={() => requestSort('country')}
                                >
                                    מדינה {getSortIndicator('country')}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
